const { getDb } = require('./__db') // Updated import
...
class Channel {
  ...
  async get (name) {
    ...
    log('Collection:', collection.collectionName)
    const channels = await collection.find({ name }).toArray()
    log('Channels found:', channels)
    return channels
  }
  ...
  async save (channelData) {
    ...
    const idToQuery = channelData._id ? channelData._id : undefined
    return this._getCollection().replaceOne({ _id: idToQuery }, channelData, {
      upsert: true
    })
  }
  ...
  async getRandomButton (channelName) {
    ...
    if (!channelName) return null
    const results = await this._getCollection()
      .aggregate([
